 ...
 * @param selectedIndex Index into `rows`; -1 when nothing is selected.
 * @param onSelect Fired with the clicked index.
...
export function PhotoFilmstrip({
  ...
  // The visible window, widened to always contain the selection: `scrollIntoView`
  // below needs the selected cell mounted, and after a Home/End jump the scroll
  // position still describes the *old* window.
  const [start, end] = useMemo(() => {
    if (rows.length === 0) return [0, 0] as const
    const width = viewWidth > 0 ? viewWidth : step * 8
    let from = Math.floor(scrollLeft / step) - OVERSCAN
    let to = Math.ceil((scrollLeft + width) / step) + OVERSCAN
    if (selectedIndex >= 0) {
      from = Math.min(from, selectedIndex - 2)
      to = Math.max(to, selectedIndex + 3)
    }
    return [Math.max(0, from), Math.min(rows.length, to)] as const
  }, [rows.length, scrollLeft, viewWidth, step, selectedIndex])
  ...
  const visible: StripRow[] = rows.slice(start, end)
          ...
          return (
            <UnstyledButton
              ...
              style={{
                ...
                boxShadow: selected ? `0 0 0 ${RING}px ${alpha(VX.neutral, 0.8)}` : 'none',
